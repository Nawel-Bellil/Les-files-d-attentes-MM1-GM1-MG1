...
class GM1Simulator:
    ...
    def simulate(self):
        ...
        results = {
            ...
            'mu': self.mu_rate,
            'rho_emp': rho_emp,
            'rho_theo': theoretical_values['rho_theo'],
...
def run_gm1_experiments():
    ...
    for lambda_rate in lambda_values:
        ...
        avg_result = {
            ...
            'rho_theo': repetition_results[0]['rho_theo'],  # Identique pour toutes les répétitions
...
def save_results_and_create_graphs(results):
    ...
    # Graphique 1: Taux d'occupation ρ
    axes[0, 0].plot(df['lambda'], df['rho_emp'], 'o-', color=emp_color, 
                   linewidth=2, markersize=7, label='ρ Empirique')
    axes[0, 0].plot(df['lambda'], df['rho_theo'], 's--', color=theo_color, 
                   linewidth=2, markersize=7, label='ρ Théorique')
    ...
    # Affichage du tableau récapitulatif avec les colonnes demandées
    print(f"\n📋 TABLEAU RÉCAPITULATIF G/M/1 - THÉORIQUE VS EMPIRIQUE:")
    print("=" * 120)
    print(f"{'λ':<5} {'μ':<5} {'ρ_emp':<8} {'ρ_theo':<8} {'T_rép_emp':<12} {'T_rép_theo':<12} "
          f"{'T_att_emp':<12} {'T_att_theo':<12} {'Util_emp':<10} {'Util_theo':<10} {'Clients':<10} {'Temps_sim':<10}")
    print("-" * 120)
    
    for _, row in df.iterrows():
        print(f"{row['lambda']:<5.1f} {row['mu']:<5.1f} {row['rho_emp']:<8.4f} {row['rho_theo']:<8.4f} "
...
if __name__ == "__main__":
    ...
    print(f"   lambda, mu, rho_emp, rho_theo, avg_response_time_emp, avg_response_time_theo,")
